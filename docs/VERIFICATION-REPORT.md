...
### ✅ [검증 9] Playwright 실 브라우저 E2E 전수 테스트 결과
Playwright(Chromium Headless)를 구동하여 실제 Next.js 프로덕션 서버(`http://localhost:3000`)를 대상으로 모든 화면 진입, 탭 전환, 폼 입력/제출, 다운로드 라우트 응답을 실제 브라우저 인터랙션으로 검증하였습니다.

| 테스트 스위트 | 검증 항목 | 세부 동작 및 확인 내용 | 결과 |
|---|---|---|---|
| **1. 대시보드 메인 (`/`)** | 상단 긴급/지연 알림 카드 | 마감 D-3 및 지연 항목 9개 카드 정상 렌더링 확인 | **PASS** |
| | 월간 캘린더 네비게이션 | 이전달/다음달 이동 버튼 클릭 시 `?month=2026-08` URL 파라미터 갱신 및 캘린더 리렌더링 | **PASS** |
| **2. 캠페인 허브 연계** | 허브 내 행사 배너 | `/campaigns/[id]` 상단 행사 배너 클릭 시 `/events` 목록으로 정상 라우팅 | **PASS** |
| **3. 서브프로젝트 B (행사 관리)** | 직접 초청자 추가 | 인플루언서 이름 입력 및 [추가] 클릭 시 실시간 목록 추가 확인 | **PASS** |
| | 당일 체크인 토글 | 현장 참석 체크박스 클릭 인터랙션 정상 확인 | **PASS** |
| | 운영안 웹 편집 & 저장 | 플레이스홀더 폼 값 수정 후 [운영안 저장] 토스트 확인 | **PASS** |
| | 체크리스트 항목 등록 | 새 할 일 등록 및 행 렌더링 확인 | **PASS** |
| | 운영안 PPT 다운로드 라우트 | `/campaigns/[id]/events/[eventId]/plan/export` HTTP 200 및 `application/vnd.openxmlformats-...` MIME 타입 확인 | **PASS** |
| **4. 서브프로젝트 C (SNS 운영)** | SNS 계정 상세 허브 | 계정 정보, 상단 누적 성과 카드, 2종 공개 토큰 복사 버튼 렌더링 확인 | **PASS** |
| | 탭 전환 & 사전설문 열람 | [사전설문 응답 결과] 및 [콘텐츠 목록] 탭 전환 정상 확인 | **PASS** |
| | 새 콘텐츠 기획 등록 | 기획안 모달에서 주제 입력 후 등록 시 목록 카드 즉시 반영 확인 | **PASS** |
| | SNS 운영안 편집 & PPT 다운로드 | `/sns/[id]/plan` 저장 및 `/export` HTTP 200 다운로드 라우트 확인 | **PASS** |
| **5. 비인증 공개 라우트** | `/sns-intake/[token]` | 광고주 사전설문 폼 입력 및 제출 시 성공 안내 화면 전환 확인 | **PASS** |
| | `/sns-approval/[token]` | 시안 열람, 수정의견 입력 후 [수정 요청] 피드백 실시간 저장 및 화면 표시 확인 | **PASS** |
| **6. 전역 설정 페이지** | `/settings/sns-intake` | 사전설문 기본 질문틀 관리 페이지 정상 렌더링 | **PASS** |
| | `/settings/ppt-templates` | 공용 PPT 템플릿 관리 페이지 정상 렌더링 | **PASS** |
| **7. 서브프로젝트 A 공개 라우트** | `/apply/[token]` | 인플루언서 체험단 지원폼 정상 렌더링 확인 | **PASS** |
| | `/pre-survey/[token]` | 브랜드사 사전조사서 폼 정상 렌더링 확인 | **PASS** |
