...
> ## ✅ 완료됨 — 더 따라 하지 말 것 (2026-09-17 표시)
>
> **여기 적힌 일은 끝났다.** B(인플루언서 행사) · C(SNS 운영) · D(오버뷰·전체일정)는
> 결국 **다른 코드베이스가 아니라 이 저장소에** 구현됐고, 배포되어 실사용 중이다.
> `app/(dashboard)/campaigns/[id]/events/`, `app/(dashboard)/sns/`,
> `app/(dashboard)/page.tsx` 와 `lib/overview/` 가 그 결과물이다.
>
> **아래 내용은 지금 사실이 아니다.** 기록으로 남길 뿐이니 그대로 실행하지 말 것:
>
> | 문서의 전제 | 지금 |
> |---|---|
> | 저장소가 `nayounglee0924/marketing` | `grapevine26/marketing_dashboard` |
> | B/C/D를 다른 코드베이스에서 구현 | **이 저장소에 구현 완료** |
> | `AUTH_DISABLED` 로 로그인을 꺼둠 | 로그인·승인·등급이 정상 동작한다 |
> | RLS 가 `to authenticated` | 정책 0개 전면 차단. 앱은 service_role 로만 붙는다 |
> | `createDashboardSupabaseClient` 라는 임시 장치 | 없다. 데이터 조회는 `lib/db/client.ts` 의 `db()` 한 곳으로 모았다 |
> | (그 밖의 클라이언트) | 비밀번호 검증용 하나뿐(`lib/auth/profile.ts`). 데이터는 읽지 않는다 |
>
> 지우지 않고 남겨 둔 이유는 **왜 그때 그런 선택을 했는지**가 여기에만 있어서다.
> 지금 코드를 알고 싶으면 `docs/HANDOFF.md` 를 읽을 것.

작성: 2026-09-01 (완료 표시: 2026-09-17)
