    ...
    <div className="space-y-6 max-w-5xl mx-auto">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div className="space-y-1">
          <Link href={`/campaigns/${campaign.id}/reports`} className="text-xs text-slate-400 hover:text-white inline-flex items-center gap-1 mb-1 transition">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-5 gap-3">
        {[
          { label: "총 지원자", value: `${metrics.totalApplicants}명`, cls: "text-white" },
          ...
          <div key={m.label} className="p-4 rounded-2xl bg-slate-900 border border-slate-800">
      ...
      <div className="p-6 rounded-2xl bg-slate-900 border border-slate-800 space-y-4">
        <h2 className="text-base font-bold text-white">참여 인플루언서 성과 목록 (생성 시점 스냅샷)</h2>
          ...
          <table className="w-full text-left text-xs">
            ...
            <tbody className="divide-y divide-slate-800 text-slate-300">
              {selectedApplicants.length === 0 ? (
                <tr>
                  <td colSpan={6} className="p-6 text-center text-slate-500">선정된 인플루언서가 없습니다.</td>
                </tr>
              ) : (
