    ...
    <div className="space-y-6 max-w-6xl mx-auto font-sans">
      ...
      {events.length === 0 ? (
        <div className="p-12 text-center border border-dashed border-[#22242A] rounded-2xl bg-[#131418] space-y-3">
          <p className="text-zinc-400 text-xs">등록된 행사가 없습니다.</p>
        </div>
      ) : (
          ...
          {events.map(async (ev) => {
            ...
            return (
              <Link
                key={ev.id}
                href={`/events/${ev.id}`}
                className="group p-5 rounded-2xl bg-[#131418] border border-[#22242A] hover:border-indigo-500/40 hover:bg-[#181A20] transition flex flex-col justify-between space-y-4"
              >
                ...
                <div className="pt-3 border-t border-[#22242A] flex items-center justify-between text-xs">
                  <div className="flex items-center gap-3 text-zinc-400">
                    <span>참석 확정: <strong className="text-blue-400">{attendingCount}</strong>/{ev.capacity}명</span>
                    <span>체크인: <strong className="text-emerald-400">{checkedInCount}</strong>명</span>
                  </div>
