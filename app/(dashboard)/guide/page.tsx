...
const SHARE_LINKS = [
  {
    ...
    to: "광고주",
    can: "브랜드·제품 정보를 답변. [AI 추천받기]로 초안 작성 가능 (질문마다 하루 3회)",
  ...
  },
  {
    ...
    to: "광고주",
    can: "톤앤매너·금기 키워드 답변. [AI 추천 답변] 사용 가능 (질문마다 하루 3회)",
    hidden: "콘텐츠 목록, 성과 수치",
  },
  {
    group: "SNS 운영",
...
const TROUBLE = [
  ...
  },
  {
    ...
    a: "지원폼은 사람이 눌러서 닿을 수 없는 자리(링크 하나당 10분에 1000건)에 상한이 있어 테스트 몇 번으로는 막히지 않습니다. 막히는 쪽은 광고주에게 보내는 사전조사·사전설문 링크로, 같은 접속 위치에서 10분에 10회, 링크 전체로는 1시간에 100회까지입니다. 10분(링크 기준은 1시간) 뒤 자동으로 풀립니다. 필수값 누락처럼 접수되지 않은 제출은 횟수를 도로 돌려주므로, 오타로 여러 번 다시 써도 막히지 않습니다.",
  },
  {
  ...
  },
  {
    ...
    a: "보고서는 만든 시점의 스냅샷을 보존합니다. 그 뒤로 값이 달라졌으면 보고서 화면 위에 무엇이 달라졌는지 알려 줍니다. 최신 수치가 필요하면 목록에서 새 결과보고서를 생성하고, 헷갈리지 않게 옛 보고서는 제목을 바꾸거나 삭제하세요.",
  },
];
      ...
      <section id="before" className="space-y-4">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          ...
          <div className="p-5 rounded-2xl bg-surface border border-border space-y-2">
            ...
            <p className="text-xs text-text-2 leading-relaxed">
              광고주에게 보내는 <strong>사전조사·사전설문</strong> 링크는 같은 접속 위치에서 <strong>10분에 10회</strong>,
              링크 전체로는 <strong>1시간에 100회</strong>까지 제출됩니다. 담당자가 테스트하다 막힐 수 있는데 고장이 아니라 스팸
              방지 장치이며 시간이 지나면 자동으로 풀립니다. <strong>인플루언서 지원폼은 상한이 훨씬 높아</strong>(링크 하나당
              10분에 1000건) 모집이 몰려도 스스로 닫히지 않습니다.
            </p>
          </div>
        </div>
      </section>
        ...
        <div className="space-y-4">
          ...
          <div className="p-5 rounded-2xl bg-surface border border-border space-y-3">
            ...
            <div className="text-xs text-text-2 space-y-1.5 pl-8.5 leading-relaxed">
              ...
              <p>• 보고서 화면 위에 <strong>어느 시점의 스냅샷인지</strong>가 적히고, 그 뒤로 캠페인 값이 달라졌으면 무엇이 몇 건 달라졌는지 알려 줍니다. 지표는 옛 값인데 총평만 최신인 보고서가 그대로 나가는 것을 막기 위해서입니다.</p>
              <p>• 제목 옆 연필로 <strong>보고서 이름을 바꾸고</strong>, 목록 카드나 상세 화면에서 <strong>삭제</strong>합니다. 삭제하면 저장된 스냅샷과 총평이 함께 사라지고 되돌릴 수 없어 확인 창이 한 번 뜹니다.</p>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          <div className="p-5 rounded-2xl bg-surface border border-border space-y-2.5">
            ...
            <p className="text-xs text-text-sub leading-relaxed">
              ...
              <strong> 당일 현장 참석</strong> 칸을 체크합니다. 이름·SNS·연락처를 잘못 넣었다면 그 줄 오른쪽 끝
              <strong> 연필</strong> 버튼(이름·SNS·연락처 수정)으로 그 자리에서 고칩니다. 지우고 다시 만들 필요가 없어 RSVP와 참석 체크가
              그대로 남습니다.
            </p>
          </div>
          ...
          <div className="p-5 rounded-2xl bg-surface border border-border space-y-2.5">
            ...
            <p className="text-xs text-text-sub leading-relaxed">
              케이터링 발주, 음향 리허설처럼 준비 항목을 담당자와 마감일과 함께 적어 둡니다. 적어 둔 항목은 글자를 눌러 그 자리에서
              고치고, 마감일도 나중에 바꿀 수 있습니다. 마감일이 가까워지면 통합 오버뷰의 임박 목록과 캘린더에 자동으로 올라옵니다.
              행사 일시는 한국 시간으로 입력하고 화면에도 한국 시간으로 표시됩니다.
            </p>
          </div>
        </div>

        <p className="text-[11px] text-text-muted leading-relaxed">
          행사 상태는 준비중·행사완료·취소됨으로 바꿀 수 있고, 기본 정보 수정과 삭제도 행사 상세 화면에서 합니다.
        </p>
      </section>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
          ...
          <div className="p-4 rounded-2xl bg-surface border border-border space-y-1.5">
            ...
            <p className="text-xs text-text-2 leading-relaxed">
              ...
              JPG·PNG·WebP·GIF·MP4·WebM·MOV를 지원합니다. 첨부한 순서가 곧 광고주 화면의 캐러셀 순서라, 왼쪽 위가 1번이고
              <strong> ◀ ▶</strong> 버튼으로 앞뒤로 옮길 수 있습니다(즉시 저장). 아이폰으로 찍은 <strong>.mov</strong>는 올릴 수는
              있지만 광고주가 크롬으로 열면 검은 화면이 될 수 있어 고를 때 경고가 뜹니다. 가능하면 MP4로 변환해 올리세요.
            </p>
          </div>
          ...
          <div className="p-4 rounded-2xl bg-surface border border-border space-y-1.5">
            ...
            <p className="text-xs text-text-2 leading-relaxed">
              ...
              표시되고, 캡션을 고쳐 다시 승인대기로 올릴 수 있습니다. 광고주가 실수로 [승인]을 눌렀다면 담당자가 상태를
              <strong>승인대기</strong>로 되돌리면 그 화면에 다시 나타납니다. 되돌리는 버튼을 광고주에게 주지는 않습니다 —
              언제든 취소되는 승인은 컨펌 시점을 없애기 때문입니다.
              <strong className="block pt-1">콘텐츠를 고치면 승인이 자동으로 풀립니다.</strong> 제목·캡션·해시태그를 바꾸면
              광고주가 승인한 것과 다른 물건이 되므로 다시 승인대기로 돌아갑니다(발행 예정일만 바꾸는 것은 해당하지 않습니다).
            </p>
          </div>
          ...
          <div className="p-4 rounded-2xl bg-surface border border-border space-y-1.5">
            ...
            <p className="text-xs text-text-2 leading-relaxed">
              게시완료로 바뀐 콘텐츠에만 조회수·좋아요·댓글을 넣을 수 있습니다. 월별 집계는 <strong>발행 예정일</strong>을
              기준으로 묶입니다. 상태를 언제 바꿨는지와는 상관없으므로, 8월 콘텐츠를 9월에 뒤늦게 게시완료로 바꿔도 8월 실적에
              남고 되돌렸다 다시 올려도 달이 옮겨가지 않습니다. 월을 골라 보거나 전체 누적을 함께 봅니다.
            </p>
          </div>
        </div>
