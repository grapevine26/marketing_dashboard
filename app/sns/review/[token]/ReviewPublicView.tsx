...
export default function ReviewPublicView({ post }: { post: SnsPost }) {
  ...
  return (
    <div className="p-5 sm:p-8 rounded-3xl bg-[#131418] border border-[#22242A] space-y-5 sm:space-y-6 shadow-2xl font-sans">
      ...
      {submittedMessage ? (
        <div className="p-5 rounded-2xl bg-sky-950/30 border border-sky-800 text-center space-y-2">
          <CheckCircle2 className="w-8 h-8 text-sky-400 mx-auto" />
          <p className="text-xs sm:text-sm text-sky-300 font-medium leading-relaxed">{submittedMessage}</p>
        </div>
      ) : (
        <div className="space-y-4 pt-2 border-t border-[#22242A]">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 pt-2">
            <button
              ...
              className="py-3 rounded-2xl bg-[#181A20] hover:bg-[#22242A] text-zinc-200 text-xs font-semibold border border-[#22242A] transition disabled:opacity-50 active:scale-95 order-2 sm:order-1"
            >
              수정 요청 전달하기
            </button>
            <button
              ...
              className="py-3 rounded-2xl bg-sky-600 hover:bg-sky-500 text-white text-xs font-bold shadow-lg shadow-sky-500/25 transition disabled:opacity-50 active:scale-95 order-1 sm:order-2"
            >
              시안 승인 (컨펌 완료) ✓
            </button>
          </div>
